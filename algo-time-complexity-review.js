...
function partialContains(array, target, start){
  return array.slice(start).indexOf(target) > -1; // slice = linear, indexOf = linear
}
...
// linear (On)
//what if we replace 10 with a parameter? (quad - n^2)
...
// logOn -> you have to divide by the same thing each time for it to be log
...
// logOn - taking the quad tree into 4ths each time
